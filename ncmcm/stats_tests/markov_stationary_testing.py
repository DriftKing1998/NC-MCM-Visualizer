...
def test_params_s(axes, reps, N_states):
    ...
    for i in range(reps):
        true_seq = simulate_markovian(M=3000, N=N_states, order=1)
        rand_seq = simulate_random_sequence(M=3000, N=N_states)
        lag2_seq = simulate_markovian(M=3000, N=N_states, order=2)
        not_stat = non_stationary_process(M=3000, N=N_states, changes=10)

        x, adj_x = stationarity(true_seq, plot=False, sim_stationary=800)
        y, adj_y = stationarity(rand_seq, plot=False, sim_stationary=800)
        z, adj_z = stationarity(lag2_seq, plot=False, sim_stationary=800)
        a, adj_a = stationarity(not_stat, plot=False, sim_stationary=800)
