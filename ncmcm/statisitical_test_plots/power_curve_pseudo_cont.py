        ...
        for x, order in enumerate(orders):
            if orders[order] == 0:
                ...
                if orders[order] == 1:
                    ...
                    seq = nc.pseudo_cont_non_stationary_process(N=states, M=l, changes=5,
                                                                epsilon=(1 - ((orders[order] - 1) * 0.2)) / 10)
...
with pd.ExcelWriter(f"StationaryEpsilon_TT_ES_{states}_{cycles}_{sims}_{alpha}.xlsx", engine="openpyxl") as writer:
...
with pd.ExcelWriter(f"StationaryEpsilon_KS_ES_{states}_{cycles}_{sims}_{alpha}.xlsx", engine="openpyxl") as writer:
...
with pd.ExcelWriter(f"StationaryEpsilon_TT_REJ_{states}_{cycles}_{sims}_{alpha}.xlsx", engine="openpyxl") as writer:
...
with pd.ExcelWriter(f"StationaryEpsilon_KS_REJ_{states}_{cycles}_{sims}_{alpha}.xlsx", engine="openpyxl") as writer:
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f'Epsilon_{states}_{cycles}_{sims}_{alpha}.png')
plt.close()

# with pd.ExcelWriter(f"StationaryEpsilon_KS_STD_{states}_{cycles}_{sims}.xlsx", engine="openpyxl") as writer:
...
for idx_l, l in enumerate(lengths):
    ...
    for y in range(cycles):
        ...
        for x, order in enumerate(orders):
            ...
            p_ks, ks, p_t, tt = nc.stationary_property_test(seq, simulations=sims, num_states=states, test_mode='both')
            effect_ks[x, y] = ks
            effect_tt[x, y] = tt
            rejections_ks[x, y] = 1 if p_ks < alpha else 0
            rejections_tt[x, y] = 1 if p_t < alpha else 0
