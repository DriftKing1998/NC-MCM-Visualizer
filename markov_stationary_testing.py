...
def test_params_s(axes, parts=10, reps=3, N_states=10):
    ...
    for p in range(parts-1):
        ...
        for i in range(reps):
            ...
            x, adj_x = test_stationarity(true_seq, parts=p + 2, plot=False, test_statistics=800)
            y, adj_y = test_stationarity(rand_seq, parts=p + 2, plot=False, test_statistics=800)
            z, adj_z = test_stationarity(lag2_seq, parts=p + 2, plot=False, test_statistics=800)
            a, adj_a = test_stationarity(not_stat, parts=p + 2, plot=False, test_statistics=800)
            b, adj_b = test_stationarity(worm_seq, parts=p + 2, plot=False, test_statistics=800)
...
fig.suptitle(f'Mean p-values (CI 75%) of Stationarity Test for 2 to {parts} chunks')
plt.show()
